...
export class _Burned extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id !== null, "Cannot save _Burned entity without an ID");
    assert(
      id.kind == ValueKind.STRING,
      "Cannot save _Burned entity with non-string ID. " +
        'Considering using .toHex() to convert the "id" to a string.'
    );
    store.set("_Burned", id.toString(), this);
  }

  static load(id: string): _Burned | null {
    return store.get("_Burned", id) as _Burned | null;
  }
